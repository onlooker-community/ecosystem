...
_CARTOGRAPHER_EVENTS_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${_CARTOGRAPHER_EVENTS_LIB_DIR}/substrate-resolve.sh" ]]; then
	# shellcheck source=plugins/cartographer/scripts/lib/substrate-resolve.sh
	source "${_CARTOGRAPHER_EVENTS_LIB_DIR}/substrate-resolve.sh"
fi
