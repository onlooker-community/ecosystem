...
_ASSAYER_EVENTS_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${_ASSAYER_EVENTS_LIB_DIR}/substrate-resolve.sh" ]]; then
	# shellcheck source=plugins/assayer/scripts/lib/substrate-resolve.sh
	source "${_ASSAYER_EVENTS_LIB_DIR}/substrate-resolve.sh"
fi
