...
_ECHO_EVENTS_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${_ECHO_EVENTS_LIB_DIR}/substrate-resolve.sh" ]]; then
	# shellcheck source=plugins/echo/scripts/lib/substrate-resolve.sh
	source "${_ECHO_EVENTS_LIB_DIR}/substrate-resolve.sh"
fi
