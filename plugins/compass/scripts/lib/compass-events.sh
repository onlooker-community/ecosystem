...
_COMPASS_EVENTS_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${_COMPASS_EVENTS_LIB_DIR}/substrate-resolve.sh" ]]; then
	# shellcheck source=plugins/compass/scripts/lib/substrate-resolve.sh
	source "${_COMPASS_EVENTS_LIB_DIR}/substrate-resolve.sh"
fi
