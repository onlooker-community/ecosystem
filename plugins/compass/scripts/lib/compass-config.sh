...
_COMPASS_CONFIG_LOADER="${_COMPASS_CONFIG_LIB_DIR}/config-loader.sh"
if [[ ! -f "$_COMPASS_CONFIG_LOADER" ]]; then
	printf 'compass: missing %s — plugin package is incomplete\n' \
		"$_COMPASS_CONFIG_LOADER" >&2
	exit 1
fi
# shellcheck source=plugins/compass/scripts/lib/config-loader.sh
source "$_COMPASS_CONFIG_LOADER"

_compass_CONFIG="{}"

compass_config_load() {
	local cwd="${1:-}"
	config_load_plugin "compass" "$cwd" "_compass_CONFIG"
	return 0
}
