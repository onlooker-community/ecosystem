...
_WARDEN_EVENTS_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${_WARDEN_EVENTS_LIB_DIR}/substrate-resolve.sh" ]]; then
	# shellcheck source=plugins/warden/scripts/lib/substrate-resolve.sh
	source "${_WARDEN_EVENTS_LIB_DIR}/substrate-resolve.sh"
fi
