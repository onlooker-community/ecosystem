...
_GOVERNOR_EVENTS_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${_GOVERNOR_EVENTS_LIB_DIR}/substrate-resolve.sh" ]]; then
	# shellcheck source=plugins/governor/scripts/lib/substrate-resolve.sh
	source "${_GOVERNOR_EVENTS_LIB_DIR}/substrate-resolve.sh"
fi
