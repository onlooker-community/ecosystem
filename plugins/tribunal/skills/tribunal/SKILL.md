...
7. **Gate.**
   ...
   ```
   If `gate.passed == true`, emit `tribunal.gate.passed` with `final_score: aggregated` and break the loop with outcome `accepted`. Otherwise emit `tribunal.gate.blocked` with the `reason`, `will_retry: (iteration_number + 1 < max_iterations)`, and `retry_iteration_number` if retrying. **Whenever `gate.failed_criterion` is present, copy it onto the event payload as `failed_criterion` — whatever the `reason` is.** The gate names the criterion on every arm of its final precedence chain — `low_score`, `meta_override`, `dissent_unresolved`, and `criterion_floor` — not only when `criterion_floor` won the contest: a criterion scored far below its floor also drags the aggregate under `score_threshold`, so the *worst* failures surface as `low_score`. Copying it only on `criterion_floor` drops the diagnostic precisely where it matters most. The Meta-Judge's short-circuit arms (`bias_detected`, and the `meta_override` / `dissent_unresolved` early returns) block before the criterion is attached, so an absent `failed_criterion` does not prove no floor was violated — which is why this instruction keys on the field being present rather than on the `reason`. Persist `gate.json` either way.
