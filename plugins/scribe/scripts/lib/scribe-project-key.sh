...
#
# Distinct from scribe_project_repo_root, which deliberately resolves a linked
# worktree to its parent checkout so both share a project key. That is right for
# identity and wrong for anything touching the filesystem: a worktree's files
# sit beside the parent, not beneath it, so writing an artifact "into the
# project" means writing it here (ecosystem-449.37, mirroring lineage's
# ecosystem-449.33).
scribe_worktree_root() {
	local cwd="${1:-}"
	[[ -z "$cwd" || ! -d "$cwd" ]] && return 0
