...
_COUNSEL_EVENTS_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${_COUNSEL_EVENTS_LIB_DIR}/substrate-resolve.sh" ]]; then
	# shellcheck source=plugins/counsel/scripts/lib/substrate-resolve.sh
	source "${_COUNSEL_EVENTS_LIB_DIR}/substrate-resolve.sh"
fi
