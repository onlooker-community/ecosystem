...
**Reversed after review.** This plan originally skipped a changed file when `lineage_added_content` was empty, on the reasoning that a deletion-only record carries no queryable content. Review showed the reasoning was wrong: the `Edit` path already writes exactly such a record (`lines_added: 0, lines_removed: 1`), so lineage plainly tolerates it. What the skip actually created was an asymmetry — the same deletion recorded through `Edit` and lost forever through `sed -i`, since the baseline advances either way. Task 4's fix round records both sides, passing removed content to `lineage_build_record` purely so `lines_removed` is counted; the ledger still stores added content only.
